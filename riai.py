...
def extract_and_clean_data(file_objs, card_choice, name_format):
    ...
    for file_obj in file_objs:
        ...
        for cells in rows_data:
            ...
            # --- تعديل مهم: تم تغيير شرط طول الرقم إلى 4 بدلاً من 5 لالتقاط البطاقات القديمة القصيرة ---
            card_indices = [i for i, c in enumerate(cells) if c.isdigit() and len(c) >= 4]
            ...
            # --- تعديل مهم: إضافة الدعم للرقم الواحد (عندما يكون هناك عمود "العدد" فقط قبل الاسم) ---
            elif len(digit_cells) == 1:
                withheld, eligible, total = 0, digit_cells[0], digit_cells[0]
            else:
                continue
