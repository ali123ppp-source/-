...
# -----------------------------------------------------------------------------
# النموذج السادس (المضاف حديثاً: عزل المحجوبين بالكامل - مستحق = 0)
# -----------------------------------------------------------------------------
def build_professional_word_report_v6(df, filename_base, card_choice):
    # فلترة البيانات لتبقي فقط من لديهم مستحق = 0
    df_zero = df[df["مستحق"].astype(int) == 0].copy()
    df_zero = df_zero.reset_index(drop=True)
    df_zero["ت"] = df_zero.index + 1 # إعادة الترقيم التسلسلي
    ...
    title_p = doc.add_paragraph()
    title_p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
    title_run = title_p.add_run(f"الكشف الإحصائي للمحجوبين كلياً (مستحق = 0) للوكيل: {clean_name}")
    title_run.font.name, title_run.font.size, title_run.bold = "Segoe UI Semibold", Pt(14), True
    
    headers = ["ت", "اسم رب الأسرة", "الكلي", "مستحق", "محجوب", card_choice, "ملاحظات"]
    ...
    max_name_len = max(df_zero["اسم رب الأسرة"].astype(str).str.len().max() if not df_zero.empty else 15, 15)
    dynamic_name_width = Cm(max_name_len * 0.22 + 0.5)
    col_widths = [Cm(0.9), dynamic_name_width, Cm(1.2), Cm(1.2), Cm(1.2), Cm(3.5), Cm(2.5)]
    COLOR_NAVY_BLUE = RGBColor(42, 75, 124)
    ...
    for idx, row in df_zero.iterrows():
        ...
        for i in range(7):
            row_cells[i].width = col_widths[i]
            val = row["ت"] if i==0 else row["اسم رب الأسرة"] if i==1 else row["الكلي"] if i==2 else row["مستحق"] if i==3 else row["محجوب"] if i==4 else row["رقم البطاقة"] if i==5 else "محجوب كلياً"
            text_color = RGBColor(203, 67, 53)
            format_cell_advanced(row_cells[i], val, size_pt=14, font_name="Calibri", color_rgb=text_color, align="left" if i==1 else "center")
            set_cell_background(row_cells[i], "FADBD8") # لون التنبيه الأحمر الفاتح

    return save_doc_buffer(doc, df_zero)
...
with col3:
    template_choice = st.radio(
        ...
        [
            ...
            "النموذج السادس (فلتر المحجوبين كلياً - مستحق = 0)"
        ],
        index=0,
        horizontal=False
    )
...
if st.session_state.processing_done:
    ...
    with st.spinner('جاري صياغة وهيكلة مستند Word المطور المختار...'):
        ...
        elif used_template == "النموذج الخامس (ورقة A3، حقول كبيرة، خط Uighur)":
