...
# -----------------------------------------------------------------------------
# محرك قراءة وتنظيف البيانات العبقري (النسخة المعدلة)
# -----------------------------------------------------------------------------
def extract_and_clean_data(file_obj, card_choice):
    ...
    for table in doc.tables:
        for row in table.rows:
            # 1. دمج محتوى الصف بالكامل في نص واحد لتجاوز مشكلة دمج الخلايا العشوائي
            ...
            # تخطي الترويسات والصفوف الفارغة
            if not row_text or "المركز" in row_text or "الوكيل" in row_text or "اسم رب" in row_text or "المحجوبين" in row_text:
                continue
            
            # 2. الاستخراج الذكي للاسم (التقاط أطول سلسلة حروف عربية متصلة متجاهلين أي أرقام)
            arabic_segments = re.findall(r'[\u0600-\u06FF\s]{5,}', row_text)
            if not arabic_segments: 
                continue
            full_name = max(arabic_segments, key=len).strip()
            
            # تقصير الاسم إلى 3 مقاطع صافية
            name_parts = full_name.split()
            three_part_name = " ".join(name_parts[:3])
            
            # 3. الاستخراج الذكي للأرقام وتصنيفها
            nums = re.findall(r'\d+', row_text)
            if len(nums) < 4: 
                continue # الصف غير مكتمل البيانات
                
            # تصنيف الأرقام: الطويلة هي البطاقات، القصيرة هي الإحصائيات والتسلسل
            cards = [n for n in nums if len(n) >= 5]
            smalls = [int(n) for n in nums if len(n) < 5]
            
            # تحديد البطاقات (القديمة والحديثة)
            if len(cards) >= 2:
                old_card_num = cards[0]
                new_card_num = cards[1]
            elif len(cards) == 1:
                old_card_num = new_card_num = cards[0]
            else:
                continue
                
            selected_card_num = new_card_num if card_choice == "رقم البطاقة الحديث" else old_card_num
            
            # تحديد الإحصائيات (الترتيب الثابت: محجوب، مستحق، كلي)
            if len(smalls) >= 3:
    ...
    if not df.empty:
        # حماية إضافية لإزالة أي سجلات مكررة بسبب الجداول المتداخلة
