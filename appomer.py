...
# -----------------------------------------------------------------------------
# محرك قراءة وتنظيف البيانات العبقري (النسخة الجوهرية للأنظمة المتغيرة)
# -----------------------------------------------------------------------------
def extract_and_clean_data(file_obj, card_choice):
    ...
    for table in doc.tables:
        col_map = {} # خريطة ذكية لتموضع الأعمدة
        for row in table.rows:
            # استخراج النصوص الصافية لكل خلية مع الحفاظ على هيكل الأعمدة
            cells_text = [cell.text.strip().replace('\n', ' ') for cell in row.cells]
            row_text = " ".join(cells_text)
            
            # 1. الاستكشاف الذكي (Dynamic Mapper) لترويسة الجدول لمرة واحدة
            if "اسم" in row_text and "الكلية" in row_text and not col_map:
                for i, text in enumerate(cells_text):
                    if "اسم" in text: col_map['name'] = i
                    if "الكلية" in text: col_map['total'] = i
                    if "مستحق" in text: col_map['eligible'] = i
                    if "محجوب" in text: col_map['withheld'] = i
                    if "البطاقة" in text and "الحديث" in text: col_map['new_card'] = i
                    elif "البطاقة" in text and "القديم" not in text: col_map['new_card'] = i
                    if "البطاقة القديم" in text or "البطاقةالقديم" in text.replace(" ", ""): col_map['old_card'] = i
                continue
                
            # إذا لم يتم تحديد الأعمدة بعد (أو الصف فارغ)، يتم التخطي
            if not col_map or not row_text.strip() or "المركز" in row_text or "الوكيل" in row_text:
                continue
                
            try:
                # 2. استخراج الاسم الصافي من العمود المخصص له بدقة
                name_cell = cells_text[col_map['name']]
                arabic_segments = re.findall(r'[\u0600-\u06FF\s]{4,}', name_cell)
                if not arabic_segments: 
                    continue
                full_name = max(arabic_segments, key=len).strip()
                
                # 3. المعالجة العبقرية للأسماء المركبة لحماية الهوية
                ...
                for p in name_parts:
                    ...
                    # لا نحسب المقطع ككلمة مستقلة إذا كان من ضمن البوادئ المركبة
                    if p not in compound_prefixes:
                        real_words += 1
                    if real_words == 3: # نتوقف عند اكتمال 3 كلمات حقيقية
                        break
                three_part_name = " ".join(final_parts)
                
                # 4. الاستخراج الدقيق للبطاقات من أعمدتها
                new_c = ""
                if 'new_card' in col_map:
                    n_cards = re.findall(r'\d{5,}', cells_text[col_map['new_card']])
                    if n_cards: new_c = n_cards[0]
                    
                old_c = ""
                if 'old_card' in col_map:
                    o_cards = re.findall(r'\d{5,}', cells_text[col_map['old_card']])
                    if o_cards: old_c = o_cards[0]
                    
                # نظام الطوارئ في حال دمج البطاقات بخلايا غير متوقعة
                if not new_c and not old_c:
                    all_cards = re.findall(r'\d{5,}', row_text)
                    if len(all_cards) >= 2:
                        old_c, new_c = all_cards[0], all_cards[1]
                    elif len(all_cards) == 1:
                        new_c = old_c = all_cards[0]
                        
                selected_card_num = new_c if card_choice == "رقم البطاقة الحديث" else old_c
                if not selected_card_num:
                    selected_card_num = old_c if old_c else new_c
                    
                if not selected_card_num: continue
                
                # 5. استخراج الإحصائيات مباشرة من أعمدتها (لحماية الكود من أرقام الملاحظات)
                def get_stat(key):
                    if key in col_map:
                        nums = re.findall(r'\d+', cells_text[col_map[key]])
                        if nums: return int(nums[0])
                    return 0
                    
                total = get_stat('total')
                eligible = get_stat('eligible')
                withheld = get_stat('withheld')
                
                # نظام طوارئ للإحصائيات 
                if total == 0 and eligible == 0:
                    smalls = [int(n) for n in re.findall(r'\d+', row_text) if len(n) < 5]
                    if len(smalls) >= 3:
    ...
    if not df.empty:
        # إزالة أي قيود مكررة لضمان النظافة التامة
...
if st.button("⚙️ تشغيل محرك التنظيم والتنسيق المتقدم الكلي"):
    if uploaded_file:
        with st.spinner('جاري قراءة وتحليل البيانات بذكاء وإعداد التنسيق...'):
            try:
