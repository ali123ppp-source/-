...
def _extract_records_by_headers(rows_data, card_choice, name_length_choice, external_idx_map=None, _out_idx_map=None, swap_old_new_cards=False):
    ...
    for i in range(start_row, len(rows_data)):
        ...
        # التبديل يُطبَّق فقط عند وجود القيمتين معاً بالصف (زوج قديم/حديث حقيقي
        # معكوس التسمية، كما تحقّق المستخدم من نموذج ورقي فعلي) — أما صف لا
        # يحمل إلا رقماً واحداً (كأسرة جديدة سُجِّلت بالبطاقة الموحّدة الحديثة
        # فقط بلا رقم قديم إطلاقاً) فتبديله يضع الرقم الوحيد الموجود في الحقل
        # الخطأ، فيُستبعد من التبديل ويبقى كما صُنِّف أصلاً.
        if swap_old_new_cards and old_card and new_card:
            old_card, new_card = new_card, old_card
        ...
        records.append({
            ...
            **card_fields,
            ...
            "محجوب": get_num("محجوب"),
            "مستحق": get_num("مستحق"),
            "_old_card_sort": int(old_card) if old_card else 10**12,
        })
        ...
        if not df.empty:
            if sort_by_card_only:
                ...
                if sort_by_card_within_group:
                    ...
                    df = df.drop(columns=["_grp_letter"])
                else:
    ...
    for cells in rows_data:
        ...
        raw_records.append({
            ...
            **card_fields,
            ...
            "محجوب": withheld,
            "مستحق": eligible,
            "_old_card_sort": int(old_card_num) if old_card_num else 10**12,
        })
...
def build_professional_word_report(df, filename_base, card_choice, sort_alphabetically=True, visible_totals=None):
    ...
    for idx, row in df.iterrows():
        ...
        def _v1_cell_value(i, row=row, is_eligible_zero=is_eligible_zero, show_children=show_children, offset=offset):
            if i == 0: return row["ت"]
            if i == 1: return row["اسم رب الأسرة"]
            if i == 2: return "x" if is_eligible_zero else ""
            if i == 3: return row["الكلي"]
            if show_children and i == 4: return row["اطفال"]
            if i == 4 + offset: return row["مستحق"]
            if i == 5 + offset: return row["محجوب"]
            if i == 6 + offset: return row["رقم البطاقة"]
            if i == 7 + offset: return "محجوب" if is_eligible_zero else ""
            return ""
        ...
        if is_combined:
            ...
            for i in remaining_indices:
                ...
                font_size = 14 if i==5+offset else 12 if i==7+offset and is_eligible_zero else 16
                text_color = RGBColor(203, 67, 53) if i==7+offset and is_eligible_zero else (RGBColor(230, 126, 34) if show_children and i==4 and int(val) > 0 else None)
                if i == 1:
                ...
                if is_eligible_zero: set_cell_background(cell, "EC7063")
                    ...
                    elif show_children and i==4: set_cell_background(cell, "FDEBD0")
                    elif i==4+offset: set_cell_background(cell, "E8F8F5")
...
def build_professional_word_report_v2(df, filename_base, card_choice, sort_alphabetically=True, visible_totals=None):
    ...
    for idx, row in df.iterrows():
        ...
        def _v2_cell_value(i, row=row, is_eligible_zero=is_eligible_zero, show_children=show_children, offset=offset):
            if i == 0: return row["ت"]
            if i == 1: return row["اسم رب الأسرة"]
            if i in (2, 3): return "x" if is_eligible_zero else ""
            if i == 4: return row["الكلي"]
            if show_children and i == 5: return row["اطفال"]
            if i == 5 + offset: return row["مستحق"]
            if i == 6 + offset: return row["محجوب"]
            if i == 7 + offset: return row["رقم البطاقة"]
            if i == 8 + offset: return "محجوب" if is_eligible_zero else ""
            return ""
        ...
        if is_combined:
            ...
            for i in remaining_indices:
                ...
                text_color = RGBColor(203, 67, 53) if i==8+offset and is_eligible_zero else (RGBColor(230, 126, 34) if show_children and i==5 and int(val) > 0 else None)
                if i == 1:
                ...
                if is_eligible_zero: set_cell_background(cell, "EC7063")
                    ...
                    elif show_children and i==5: set_cell_background(cell, "FDEBD0")
                    elif i==5+offset: set_cell_background(cell, "E8F8F5")
...
def build_professional_word_report_v3(df, filename_base, card_choice, sort_alphabetically=True, visible_totals=None):
    ...
    for idx, row in df.iterrows():
        ...
        def _v3_cell_value(i, row=row, show_children=show_children, offset=offset):
            if i == 0: return row["ت"]
            if i == 1: return row["اسم رب الأسرة"]
            if i == 2: return row["رقم البطاقة"]
            if i == 3: return row["الكلي"]
            if show_children and i == 4: return row["اطفال"]
            if i == 4 + offset: return row["مستحق"]
            if i == 5 + offset: return row["محجوب"]
        ...
        if is_combined:
            ...
            for i in remaining_indices:
                ...
                text_color = RGBColor(230, 126, 34) if show_children and i==4 and int(val) > 0 else None
                if i == 1:
                    format_name_cell_with_small_suffix(cell, val, base_size=16, small_size=10, font_name="Calibri", color_rgb=text_color, align="left")
                else:
                    format_cell_advanced(cell, val, size_pt=16, font_name="Calibri", color_rgb=text_color, align="left" if i==1 else "center")
                if is_eligible_zero: set_cell_background(cell, "EC7063")
                if i == 3: set_cell_background(cell, "E5E7E9")
                if show_children and i == 4: set_cell_background(cell, "FDEBD0")
...
# --- النموذج الثالث عشر: تصميم فاخر بلوحة ألوان أحادية (كحلي/ذهبي) بديلاً عن
# الألوان المتعددة الصارخة بالنماذج الأخرى — أثبت بحث اتجاهات 2026 أن لوحة
# محدودة بعمق تدرّجي واحد تبدو أرقى وأكثر احترافية من مزيج ألوان "قوس قزح"،
# مع الاحتفاظ بالتحذير الأحمر القوي لحالة "مستحق=0" فقط لأنه استثناء حقيقي
# يجب أن يبقى لافتاً بصرياً بصرف النظر عن رصانة باقي التصميم.
def build_professional_word_report_v13(df, filename_base, card_choice, sort_alphabetically=True, visible_totals=None):
    doc = Document()
    setup_document_layout(doc, filename_base)

    NAVY_INK = RGBColor(15, 42, 68)
    ...
    title_run = title_p.add_run(f"الكشف الإحصائي المنسق للوكيل: {clean_name}")
    title_run.font.name, title_run.font.size, title_run.bold = "Segoe UI Semibold", Pt(14), True
    title_run.font.color.rgb = NAVY_INK

    is_combined = card_choice == "القديم والحديث"
    orig_headers = ["ت", "اسم رب الأسرة", "الكلي", "مستحق", "محجوب", card_choice, "ملاحظات"]
    max_name_len = max(df["اسم رب الأسرة"].astype(str).str.len().max(), 15)
    dynamic_name_width = Cm(max_name_len * 0.22 + 0.5)
    col_widths = [Cm(0.9), dynamic_name_width, Cm(1.1), Cm(1.1), Cm(1.1), Cm(1.8), Inches(1.0)]
    remaining_indices = [1, 2, 3, 4, 6] if is_combined else None
    ...
    table = doc.add_table(rows=1, cols=len(headers))
    table.style, table.alignment = 'Table Grid', WD_TABLE_ALIGNMENT.CENTER
    set_table_borders(table, color_hex="1B3A63")
    table._tbl.tblPr.append(parse_xml(f'<w:bidiVisual {nsdecls("w")}/>'))
    table.rows[0]._tr.get_or_add_trPr().append(parse_xml(f'<w:tblHeader {nsdecls("w")}/>'))
    table.rows[0].height = Inches(0.7)

    def _style_header_cell(cell, title, width):
        cell.width, cell.vertical_alignment = width, WD_ALIGN_VERTICAL.CENTER
        format_cell_advanced(cell, title, bold=True, size_pt=13, font_name="Segoe UI Semibold", align="center", color_rgb=RGBColor(255, 255, 255))
        set_cell_background(cell, "0F2A44")

    if is_combined:
        hdr_cells = table.rows[0].cells
        _style_header_cell(hdr_cells[0], "ت", col_widths[0])
        j = 1
        for label in ["القديم", "الحديث"]:
            _style_header_cell(hdr_cells[j], label, Cm(1.8))
            j += 1
        for i in remaining_indices:
            _style_header_cell(hdr_cells[j], orig_headers[i], col_widths[i])
            j += 1
    else:
        for target_col, i in enumerate(keep_indices):
            _style_header_cell(table.rows[0].cells[target_col], orig_headers[i], col_widths[i])
    ...
    for idx, row in df.iterrows():
        ...
        new_row._tr.get_or_add_trPr().append(parse_xml(f'<w:cantSplit {nsdecls("w")}/>'))
        is_eligible_zero = int(row["مستحق"]) == 0
        base_bg = ALERT if is_eligible_zero else (ZEBRA if zebra_on else "FFFFFF")
        zebra_on = not zebra_on

        def _v13_cell_value(i, row=row, is_eligible_zero=is_eligible_zero):
            if i == 1: return row["اسم رب الأسرة"]
            if i == 2: return row["الكلي"]
            if i == 3: return row["مستحق"]
            if i == 4: return row["محجوب"]
            if i == 6: return "محجوب" if is_eligible_zero else ""
            return ""

        def _fill_ter_cell(cell, width):
            cell.width, cell.vertical_alignment = width, WD_ALIGN_VERTICAL.CENTER
            format_cell_advanced(cell, row["ت"], bold=True, size_pt=9.6, font_name="Calibri", align="center", color_rgb=NAVY_INK)
            set_cell_background(cell, ALERT if is_eligible_zero else GOLD)

        if is_combined:
            _fill_ter_cell(row_cells[0], col_widths[0])
            j = 1
            for card_key in ["رقم البطاقة القديم", "رقم البطاقة الحديث"]:
                cell = row_cells[j]
                cell.width, cell.vertical_alignment = Cm(1.8), WD_ALIGN_VERTICAL.CENTER
                format_cell_advanced(cell, row[card_key], size_pt=16, font_name="Calibri", align="center", color_rgb=NAVY_INK)
                set_cell_background(cell, base_bg)
                j += 1
            for i in remaining_indices:
                cell = row_cells[j]
                cell.width, cell.vertical_alignment = col_widths[i], WD_ALIGN_VERTICAL.CENTER
                val = _v13_cell_value(i)
                text_color = (RED_DEEP if i == 6 and is_eligible_zero else
                              GREEN_DEEP if i == 3 else RED_DEEP if i == 4 else NAVY_INK)
                if i == 1:
                    set_cell_no_wrap(cell)
                    format_name_cell_with_small_suffix(cell, val, base_size=16, small_size=10, font_name="Calibri", bold=True, color_rgb=NAVY, align="left")
                else:
                    format_cell_advanced(cell, val, bold=(i in (3, 4)), size_pt=14, font_name="Calibri", color_rgb=text_color, align="left" if i == 1 else "center")
                set_cell_background(cell, base_bg)
                j += 1
        else:
            for target_col, i in enumerate(keep_indices):
                cell = row_cells[target_col]
                if i == 0:
                    _fill_ter_cell(cell, col_widths[i])
                    continue
                cell.width, cell.vertical_alignment = col_widths[i], WD_ALIGN_VERTICAL.CENTER
                val = _v13_cell_value(i)
                text_color = (RED_DEEP if i == 6 and is_eligible_zero else
...
def build_professional_word_report_v7(df, filename_base, card_choice, sort_alphabetically=True, visible_totals=None):
    ...
    for idx, row in df.iterrows():
        ...
        def _v7_cell_value(i, row=row, show_children=show_children, offset=offset):
            if i == 0: return row["ت"]
            if i == 1: return row["اسم رب الأسرة"]
            if i == 2: return row["رقم البطاقة"]
            if i == 3: return row["الكلي"]
        ...
        if is_combined:
            ...
            for i in remaining_indices:
                ...
                if i == 1:
                    ...
                    format_cell_advanced(cell, val, size_pt=14, font_name="Calibri", color_rgb=text_color, align=cell_align)
                if is_eligible_zero:
                    set_cell_background(cell, "EC7063")
                else:
                    if i == 3: set_cell_background(cell, "EBF5FB")
...
def get_pdf_font_face_css():
    ...
    if not (regular and bold and extrabold):
        return ""
    ruqaa_css = ""
    if ruqaa_regular and ruqaa_bold:
        # خط Aref Ruqaa: خط عربي مبني على خط الرقعة (خط اليد اليومي)، يُستخدم
        # فقط بالقوالب التي تطلب صراحةً خطاً "قريباً لخط اليد" بدل Tajawal.
        ruqaa_css = f"""
            @font-face {{
                font-family: 'Aref Ruqaa';
                font-weight: 400;
                font-style: normal;
                src: url(data:font/ttf;base64,{ruqaa_regular}) format('truetype');
            }}
            @font-face {{
                font-family: 'Aref Ruqaa';
                font-weight: 700;
...
def _build_report_html_doc(df, filename_base, card_choice, template_choice, sort_alphabetically=True, agent_metadata=None, visible_totals=None):
    ...
    # طلب صريح: النموذج الحادي عشر (ثيم أخضر) يطابق تصميم الصورة المرجعية
    ...
    if template_choice == "النموذج الأول (الأصلي المطور)":
        ...
        headers = ["ت"] + card_cols + ["اسم رب الأسرة", "الكلي", "مستحق", "محجوب", "ملاحظات"] if is_combined else ["ت", "اسم رب الأسرة", "الكلي", "مستحق", "محجوب", card_choice, "ملاحظات"]
        page_size = "A4"
        page_orientation = "portrait"
    else:
    ...
    # فلتر الأطفال: يُدرج عمود "اطفال" مباشرة بجانب "الكلي" — يعمل فقط في
    # النماذج التي تعرض أعمدة الكلي/مستحق/محجوب الثلاثة منفصلة (الأول، الثاني،
    # الثالث، السابع)؛ باقي النماذج ليس فيها عمود بعنوان "الكلي" أصلاً فلا يُلمس.
    ...
    for idx, row in df.iterrows():
        ...
        if template_choice == "النموذج الأول (الأصلي المطور)":
            ...
            ter_badge = f'<span class="ter-badge">{row["ت"]}</span>'
            if is_combined:
                vals = [
                    (ter_badge, ter_bg),
                    *card_vals,
                    (display_name, "text-align: right; font-weight: bold; font-size: 14pt; color: #1B3A63;"),
    ...
    COMPACT_HEADERS = {"الكلي", "مستحق", "المستحق", "العدد المستحق", "محجوب", "المحجوب", "اطفال"}
    ...
    def _header_cell_html(h, extra_style=""):
        ...
        if h == "اطفال":
            return f'<th><span class="pill{pill_extra} pill-amber"{style_attr}>{h}</span></th>'
        if "كلي" in h:
    ...
    <head>
        ...
        <style>
            ...
            body {{
                ...
                padding-left: 0.5mm;
                padding-right: 1.8mm;
                background-image: {body_bg_image_css};
                background-size: {body_bg_size_css};
            }}
            .invoice-card {{
            ...
            .pill-amber {{ background-color: #FDEBD0; color: #B9770E; }}
            .pill-tight {{
                padding: 3px 1px;
                font-size: 10pt;
            }}
            ...
            /* طلب صريح: قالب بديل (ثيم أخضر) بنفس ترتيب أعمدة النموذج
               العاشر (ت/الاسم/المستحق/حقل فارغ) يطابق تصميم الصورة المرجعية
               — رأس وصفوف خضراء متناوبة، حواف الجدول ناعمة مدوّرة، مربع
               فارغ مستدير الحواف بدل الخلية الفارغة العادية، بخط Aref Ruqaa
               (مبني على خط الرقعة، أقرب لخط اليد) — مقصور على هذا القالب
               فقط عبر صنف table.theme-green. */
            table.theme-green, table.theme-green th, table.theme-green td {{
                font-family: 'Aref Ruqaa', 'Tajawal', 'Segoe UI Semibold', serif;
            }}
            ...
            /* النموذج الثالث عشر: لوحة أحادية كحلية/ذهبية راقية بدل الفقاعات
               الملوّنة المتعددة — رأس كحلي غامق موحّد، تناوب خفيف بدرجة كحلية
               فاتحة جداً بدل الألوان الصريحة، مع الاحتفاظ بالتحذير الأحمر
               القوي لصفوف "مستحق=0" لأنه استثناء تشغيلي حقيقي يجب أن يبقى
               لافتاً. */
        ...
        if template_choice == "النموذج الأول (الأصلي المطور)":
            ...
            if is_combined:
                vals = [
                    (row["ت"], ter_bg),
                    *card_vals,
                    (display_name, "text-align: right; font-weight: bold; color: #1B3A63;"),
                    (row["الكلي"], "font-weight: bold; color: #1B3A63;"),
                    (row["مستحق"], "font-weight: bold; color: #1E7E43;"),
                    (row["محجوب"], "font-weight: bold; color: #B02E26;"),
                    ("محجوب" if is_eligible_zero else "", "color: #B02E26; font-weight: bold;" if is_eligible_zero else "")
...
with st.container(border=True):
    ...
    TEMPLATE_HINTS = {
        ...
        "النموذج الثالث عشر (تصميم فاخر عصري)": "✨ تصميم عصري راقٍ بلوحة كحلية/ذهبية أحادية بدل الألوان المتعددة — الأنسب لمظهر أكثر فخامة.",
    }
        ...
        with st.spinner(f'جاري معالجة {order_desc}{mode_desc} وإعداد التنسيق الشرطي والمقاييس...'):
            try:
                ...
                if extracted:
                    if merge_files:
                        ...
                        if sort_by_card_only:
                            ...
                            if sort_by_card_within_group:
                                ...
                                merged_df = merged_df.drop(columns=["_grp_letter"])
                            else:
                    ...
                    for item in results:
                        if "_old_card_sort" in item["df"].columns:
                            item["df"] = item["df"].drop(columns=["_old_card_sort"])
                        if show_children_filter:
                            children_diff = item["df"]["الكلي"] - (item["df"]["مستحق"] + item["df"]["محجوب"])
                            item["df"]["اطفال"] = children_diff.clip(lower=0)
