...
def _locate_header_row(rows_data):
    ...
    for i, row in enumerate(rows_data):
        # صف ترويسة حقيقي يتكوّن من خلايا عناوين منفصلة، لا فقرة نصية واحدة طويلة
        # (مثل ملاحظات منهجية قد تحتوي بالصدفة على كلمات مشابهة لعناوين الأعمدة).
        # الحد مرتفع نسبياً (60) لتقبّل عناوين أعمدة طويلة واقعية مثل
        # "رقم البطاقة التموينية الجديدة (الحديثة)"، مع بقائه كافياً لاستبعاد فقرة نصية.
        if len(row) < 4 or any(len(cell) > 60 for cell in row):
            continue
