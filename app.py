...
# -----------------------------------------------------------------------------
# محرك قراءة وتنظيف البيانات المطور والمعدل بالكامل
# -----------------------------------------------------------------------------
def extract_and_clean_data(file_obj):
    doc = Document(file_obj)
    raw_records = []
    seen_cards = set()  # لتتبع البطاقات المستخرجة ومنع تكرار الأسطر المعكوسة
    
    raw_lines = []
    
    # 1. تجميع كافة الأسطر من الفقرات النصية (تغطية الجزء الأول الناقص)
    for p in doc.paragraphs:
        t = p.text.strip()
        if t:
            raw_lines.append(t)
            
    # 2. تجميع كافة الأسطر من الجداول (تغطية الجزء الثاني)
    for table in doc.tables:
        for row in table.rows:
            # دمج محتويات الخلايا بفاصلة موحدة لتسهيل معالجتها ديناميكياً
            t = " , ".join([cell.text.strip() for cell in row.cells if cell.text.strip()])
            if t:
                raw_lines.append(t)
                
    # 3. معالجة الأسطر ذكياً وتفادي اختلاف الهياكل والأسطر المقلوبة
    for line in raw_lines:
        # استبعاد أسطر العناوين والهيدر بكل الصيغ المحتملة
        if any(kw in line for kw in ["المركز", "الوكيل", "نوع الوكالة", "اسم رب", "رقم البطاقة", "ةلاكولا", "زكرملا"]):
            continue
            
        # استخراج اسم رب الأسرة بالاعتماد على التعبير النمطي (سلسلة من الكلمات العربية المتتالية)
        name_match = re.search(r'([\u0600-\u06FF]+(?:\s+[\u0600-\u06FF]+)+)', line)
        if not name_match:
            continue
            
        name = name_match.group(1).strip()
        
        # فلتر أمان أول: استبعاد الكلمات المقلوبة التي تبدأ بـ (ة) التاء المربوطة مثل (ةميلس، ةيريخ)
        if any(word.startswith('ة') for word in name.split()):
            continue
            
        # تقسيم السطر إلى ما قبل الاسم وما بعده لاستخراج الأرقام حسب مكانها
        idx = line.find(name)
        part_before = line[:idx]
        part_after = line[idx+len(name):]
        
        nums_before = re.findall(r'\d+', part_before)
        nums_after = re.findall(r'\d+', part_after)
        
        has_card_before = any(len(n) >= 5 for n in nums_before)
        has_card_after = any(len(n) >= 5 for n in nums_after)
        
        old_card_num = ""
        total, eligible, withheld = 0, 0, 0
        
        if has_card_before and not has_card_after:
            # التنسيق أ (الجزء الأول من الملف): أرقام البطاقات قبل الاسم، وأرقام الأفراد بعد الاسم
            cards = [n for n in nums_before if len(n) >= 5]
            if not cards: continue
            old_card_num = cards[1] if len(cards) > 1 else cards[0]
            
            counts = [int(n) for n in nums_after]
            if len(counts) >= 3:
                total, eligible, withheld = counts[0], counts[1], counts[2]
            elif len(counts) == 2:
                total, eligible, withheld = counts[0], counts[1], 0
            else:
                continue
                
        elif has_card_after:
            # التنسيق ب (الجزء الثاني من الملف): أرقام الأفراد قبل الاسم، وأرقام البطاقات بعده
            cards = [n for n in nums_after if len(n) >= 5]
            if not cards: continue
            old_card_num = cards[0]
            
            counts = [int(n) for n in nums_before]
            if len(counts) >= 3:
                withheld, eligible, total = counts[0], counts[1], counts[2]
            elif len(counts) == 2:
                withheld, eligible, total = 0, counts[0], counts[1]
            else:
                continue
        else:
            continue
            
        # فلتر أمان ثانٍ: منع التكرار والأسطر المعكوسة بمطابقة رقم البطاقة الفريد
        card_id = str(int(old_card_num))
    ...
    if not df.empty:
        df = df.drop_duplicates(subset=["رقم البطاقة القديم"]).reset_index(drop=True)
...
if st.session_state.processing_done:
    ...
    st.success(f"✅ تم التنظيم الأبجدي بنجاح لـ ({len(df_final)}) قيد اسم بدون نقصان.")
